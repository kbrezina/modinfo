 ...
 * 
 */
package com.cloudsmith.hammer.puppet.validation.graphs.dependency;
 ...
 * 
 */
public class DependencyDataCalculator implements DependencyGraphStyles {
	...
	public static class ModuleNodeData {
		public static ModuleNodeData existing(String name, String version, boolean isNode, String href) {
		...
		public static ModuleNodeData root(File root) {
			...
			return new ModuleNodeData(sb.toString(), null, ModuleType.ROOT, "");
		}

		public static ModuleNodeData unresolved(String name) {
		...
		private ModuleNodeData(String name, String version, ModuleType type, String href) {
			this.name = name;
	...
	private final IStyleFactory styles;

	private final DependencyGraphTheme theme;

	private final DotRenderer dotRenderer;

	private Injector injector;

	Map<Object, ModuleNodeData> moduleNodeData = Maps.newHashMap();

	List<ModuleEdge> moduleEdges = Lists.newArrayList();
	...
	public DependencyDataCalculator(Class<? extends IHrefProducer> hrefProducerClass, String prefix) {
		injector = Guice.createInjector(new DependencyGraphModule());
		styles = get(IStyleFactory.class);
		theme = get(DependencyGraphTheme.class);
		dotRenderer = get(DotRenderer.class);
		hrefProducer = get(hrefProducerClass); // was IHrefProducer.class
	 ...
	 * @return
	 */
	public Map<File, ModuleNodeData> calculateDependencyData(File root, Multimap<String, MetadataInfo> moduleData,
			AllModuleReferences exportData) {

		// create node data for all existing modules and check if there are ambiguities
		Multimap<String, ModuleNodeData> processedModules = ArrayListMultimap.create();
		for(MetadataInfo mi : moduleData.values()) {
			Metadata m = mi.getMetadata();
			ModuleNodeData mnd = ModuleNodeData.existing(m.getFullName(), m.getVersion(), mi.isRole(), toHREF_URL(mi));
			moduleNodeData.put(mi, mnd);
			processedModules.put(m.getFullName(), mnd);
		}
		for(String key : processedModules.keySet()) {
		...
		pptpNode = new ModuleNodeData("puppet", "n/a", ModuleType.PPTP, ""); // will not be rendered
		...
		for(MetadataInfo mi : moduleData.values()) {
			...
			for(Dependency d : mi.getUnresolvedDependencies()) {
				...
				final String requiredName = name == null
						? null
						: name.replace("/", "-").toLowerCase();
				if(processed.contains(requiredName))
					continue;
				processed.add(requiredName);
				Collection<MetadataInfo> existingVersions = moduleData.get(requiredName);
				ModuleNodeData b = null;
				if(existingVersions == null || existingVersions.size() < 1) {
					b = moduleNodeData.get(requiredName);
					if(b == null) {
						// need a node for the missing module
						b = ModuleNodeData.unresolved(name == null
								? "ERROR: Dependency without name"
								: name);
						// need to generate one that can not be found if name is null
						moduleNodeData.put(requiredName == null
	...
	private Vertex createVertexForModuleNode(ModuleNodeData mnd) {
		...
		if(mnd.version != null && mnd.version.length() > 0) {
			builder.append("\n");
				...
				for(ModuleEdge edge : m.outgoing.get(to)) {
					...
					if(edge.dependency != null)
						if(edge.dependency.getVersionRequirement() == null)
							...
							System.err.printf("%s\n", edge.dependency.getVersionRequirement().getVersion());
	...
	public String getVersionLabel(ModuleEdge edge) {
		VersionRequirement vreq = edge.dependency.getVersionRequirement();
		if(vreq == null)
			return "unversioned";
		return vreq.getMatchRule().getLiteral() + " " + vreq.getVersion();
	 ...
	 */
	public void produceGraph(ICancel cancel, File root, String title, OutputStream out, File[] roots,
			Multimap<String, MetadataInfo> moduleData, AllModuleReferences exportData) {
		...
		GraphCSS instanceRules = get(GraphCSS.class);
		instanceRules.addAll(theme.getInstanceRules());
		dotRenderer.write(cancel, out, g, theme.getDefaultRules(), instanceRules);
	}
	 ...
	 * @return
	 */
	private RootGraph produceRootGraph(ICancel cancel, String title, Multimap<String, MetadataInfo> moduleData,
	...
	public String toHREF_UNRESOLVED(String fromModuleName, String name) {
