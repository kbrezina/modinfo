...
public class SVGProducer {
	...
	/**
	 * Get instance of type via the PP RT injector.
	 * 
	 * @param <T>
	 * @param clazz
	 * @return
	...
	private <T> T get(Class<T> clazz) {
		if(injector == null)
			injector = Guice.createInjector(new FixedSVGGraphModule());
		return injector.getInstance(clazz);
	}

	/**
	 * Get an annotated instance of type via the PP RT injector.
	 * 
	 * @param <T>
	 * @param clazz
	 * @return
	 */
	private <T> T getInstance(Class<T> type, Class<? extends Annotation> option) {
		final Key<T> key = Key.get(type, option);
		return injector.getInstance(key);
	}

	// private <T> T getNamedInstance(Class<T> type, String name) {
	...
	public void produceSVG(InputStream dotStream, OutputStream svgStream, boolean compress, IProgressMonitor monitor)
		...
		IOutputStreamFilterFactory streamFilterFactory = getInstance(
			IOutputStreamFilterFactory.class, IGraphviz.SVGOutputFilter.class);
