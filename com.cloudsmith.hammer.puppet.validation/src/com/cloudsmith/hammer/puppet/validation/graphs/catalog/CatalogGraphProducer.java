...
public class CatalogGraphProducer extends AbstractCatalogGraphProducer implements CatalogGraphStyles {
	...
	public CatalogGraphProducer(Class<? extends IHrefProducer> hrefProducerClass, String prefix) {
	...
	private StyleSet labelStyleForResource(CatalogResource r, final IPath root) {
		...
		if(hasParameters || hasFooter)
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("SpacingCell", "", Span.colSpan(3))));

		labelRows.add(styles.labelRow(STYLE_ResourceTitleRow, //
			styles.labelCell(STYLE_ResourceTitleCell, builder.toString(), Span.colSpan(3))));
		...
		if(hasParameters || hasFooter) {
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("SpacingCell", "", Span.colSpan(3))));
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("HRCell", "", Span.colSpan(3))));
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("SpacingCell", "", Span.colSpan(3))));
		}
		// parameters
		for(CatalogResourceParameter a : sortedProperties) {
			...
			labelRows.add(styles.labelRow(STYLE_ResourcePropertyRow, //
				styles.labelCell(STYLE_ResourcePropertyName, a.getName()), //
				styles.labelCell(STYLE_ResourcePropertyValue, DOUBLE_RIGHT_ARROW + value, Span.colSpan(2)) //
			));
		...
		if(hasFooter) {
			...
			labelRows.add(styles.labelRow(STYLE_ResourceFileInfoRow, //
				styles.labelCell(STYLE_ResourceFileInfoCell, builder.toString(), Span.colSpan(3)).withStyles(//
					styles.tooltip(tooltip), //
					styles.href(hrefProducer.hrefToManifest(new Path(r.getFile()), root, line)) //
				)) //
			);
		}
