...
public class CatalogDeltaGraphProducer extends AbstractCatalogGraphProducer implements CatalogGraphStyles {
	...
	public CatalogDeltaGraphProducer(Class<? extends IHrefProducer> hrefProducerClass, String prefix) {
		super(hrefProducerClass, prefix);
	}
		...
		for(String propertyName : sortedNames) {
			...
			if(values.size() > 1) {
				...
				labelRows.add(styles.labelRow(
					...
					styles.labelCell(Sets.newHashSet(STYLE_ResourcePropertyValue, STYLE_Removed), DOUBLE_RIGHT_ARROW + //
				...
				labelRows.add(styles.labelRow(
					...
					styles.labelCell(Sets.newHashSet(STYLE_ResourcePropertyValue, STYLE_Added), DOUBLE_RIGHT_ARROW + //
				...
				labelRows.add(styles.labelRow(STYLE_ResourcePropertyRow, //
					createResourcePropertyMarker(marker, styleClass, renderMarkerColumnFunc), //
					styles.labelCell(Sets.newHashSet(STYLE_ResourcePropertyName, styleClass), propertyName), //
					styles.labelCell(Sets.newHashSet(STYLE_ResourcePropertyValue, styleClass), DOUBLE_RIGHT_ARROW + //
	...
	private LabelCell createResourcePropertyMarker(String marker, String styleName,
		...
		return styles.labelCell(Sets.newHashSet(STYLE_ResourcePropertyMarker, styleName), marker)//
		.withStyles(// styles.fixedSize(true),
			styles.fixedSize(true), styles.width(8), styles.height(8), styles.cellSpacing(0), //
			styles.cellPadding(0), styles.rendered(renderedFunc) //
		);
	}
	...
	private void generateEdgeDelta(RootGraph g, Set<String> edges, Map<String, Edge> oldEdgeMap,
		...
		for(String key : edges) {
			...
			if(inOld && inNew) {
				...
				e.setStyles(styles.labelFormat(styles.labelTemplate(markRemoved)));
			}
			else {
				e = newEdgeMap.get(key);
				e.addStyleClass(STYLE_Added);
				e.setStyles(styles.labelFormat(styles.labelTemplate(markAdded)));
			}
			g.addEdge(e);
		}
	}
	...
	private StyleSet labelStyleForResource(CatalogResource oldR, IPath oldRoot, CatalogResource newR, IPath newRoot,
		...
		if(hasParameters || hasFooter)
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("SpacingCell", "", Span.colSpan(1))));

		labelRows.add(styles.labelRow(STYLE_ResourceTitleRow, //
			styles.labelCell(STYLE_ResourceTitleCell, builder.toString(), Span.colSpan(1))));
		width = Math.max(width, builder.length());

		// Rendering of separator line fails in graphviz 2.28 with an error
		// labelRows.add(styles.rowSeparator());
		if(hasParameters || hasFooter) {
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("SpacingCell", "", Span.colSpan(1))));
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("HRCell", "", Span.colSpan(1))));
			labelRows.add(styles.labelRow("RowSeparator", styles.labelCell("SpacingCell", "", Span.colSpan(1))));
		}
		...
		if(innerLabelRows.size() > 0) {
			LabelCell tableCell = styles.labelCell("ResourceTableCell",//
				styles.labelTable(STYLE_ResourceTable, innerLabelRows.toArray(new LabelRow[innerLabelRows.size()])));
			labelRows.add(styles.labelRow("ResourceTableRow", tableCell));
		}
		...
		if(hasFooter) {
			...
			labelRows.add(styles.labelRow(
				...
				styles.labelCell(STYLE_ResourceFileInfoCell, builder.toString(), Span.colSpan(1)).withStyles(
					styles.tooltip(tooltip), //
					styles.href(hrefProducer.hrefToManifest(new Path(singleResource.getFile()), singleRoot, line)) //
				)) //
			);
		}
		...
		styles.labelFormat(//
		styles.labelTable(STYLE_ResourceTable, //
