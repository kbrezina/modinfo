...
import com.cloudsmith.hammer.puppet.validation.IValidationConstants;
...
import com.cloudsmith.hammer.puppet.validation.graphs.dependency.DependencyDataCalculator;
...
import com.cloudsmith.hammer.puppet.validation.runner.MetadataInfo.Resolution;
import com.cloudsmith.hammer.puppet.validation.runner.PPDiagnosticsRunner;
import com.cloudsmith.hammer.puppet.validation.runner.PuppetCatalogCompilerRunner;
import com.cloudsmith.hammer.puppet.validation.runner.PuppetCatalogCompilerRunner.CatalogDiagnostic;
import com.cloudsmith.hammer.puppet.validation.runner.RakefileInfo;
import com.cloudsmith.hammer.puppet.validation.runner.RakefileInfo.Rakefile;
import com.cloudsmith.hammer.puppet.validation.runner.RakefileInfo.Raketask;
 ...
 * @generated
 */
public class ValidationServiceImpl extends EObjectImpl implements ValidationService {
	/**
	 * 
	 */
	...
	private static void addExceptionDiagnostic(DiagnosticChain diagnostics, String message, Exception e) {
		BasicDiagnostic bd = new BasicDiagnostic(
			Diagnostic.ERROR, validationSource, ValidationServiceDiagnosticCode.INTERNAL_ERROR_VALUE, message,
			new Object[] { e });
		diagnostics.add(bd);
	...
	private static void addFileDiagnostic(DiagnosticChain diagnostics, int severity, File file, File rootDirectory,
		...
		int code = ValidationServiceDiagnosticCode.UNKNOWN_VALUE; // TODO: need "structure", rename "layout" flag
		DetailedDiagnosticData dataEntry = ValidationFactory.eINSTANCE.createDetailedDiagnosticData();
	...
	private static void addIssueDiagnostic(DiagnosticChain diagnostics, Issue issue, File processedFile,
		...
		int code = issue.isSyntaxError()
				? ValidationServiceDiagnosticCode.GEPPETTO_SYNTAX_VALUE
				: ValidationServiceDiagnosticCode.GEPPETTO_VALUE;
		DetailedDiagnosticData dataEntry = ValidationFactory.eINSTANCE.createDetailedDiagnosticData();
	...
	private static void addRubyIssueDiagnostic(DiagnosticChain diagnostics, IRubyIssue issue, File processedFile,
			File rootDirectory) {
		int code = issue.isSyntaxError()
				? ValidationServiceDiagnosticCode.RUBY_SYNTAX_VALUE
				: ValidationServiceDiagnosticCode.RUBY_VALUE;
		DetailedDiagnosticData dataEntry = ValidationFactory.eINSTANCE.createDetailedDiagnosticData();
	...
	private void addCatalogDiagnostics(DiagnosticChain diagnostics, CatalogDiagnostic d) {
		...
		int code = d.getCode() == CatalogDiagnostic.CODE_PARSE_ERROR
				? ValidationServiceDiagnosticCode.CATALOG_PARSER_VALUE
				: ValidationServiceDiagnosticCode.CATALOG_VALUE;

		// translate the data
		DiagnosticData dataEntry = ValidationFactory.eINSTANCE.createDiagnosticData();
	...
	protected EClass eStaticClass() {
		return ValidationPackage.Literals.VALIDATION_SERVICE;
	}
	...
	private Rakefile getRakefileInformation(RubyHelper rubyHelper, File f, File root, IProgressMonitor monitor) {
		...
		try {
			...
			// simply do not return any information - validation should have dealt with
			// errors
	...
	public void produceSVG(InputStream dotStream, OutputStream svgStream, boolean compress, IProgressMonitor monitor)
		...
		svgProducer.produceSVG(dotStream, svgStream, compress, monitor);
	}
	...
	public BuildResult validate(DiagnosticChain diagnostics, File source, ValidationOptions options,
		...
		if(options == null) {
			options = ValidationFactory.eINSTANCE.createValidationOptions();
	...
	private BuildResult validateDirectory(DiagnosticChain diagnostics, File root, ValidationOptions options,
		...
		for(File f : rakeFiles) {
			...
			//TODO Faulty rakefiles break the whole validation - let's just ignore them to go on...
			try {
				// parsing adds one rakefile work tick
				rakefileInfo.addRakefile(getRakefileInformation(rubyHelper, f, root, ticker.newChild(1)));
			}
			catch(Exception e) {
				//TODO Do we have any logger?
				System.out.println("Warning: Rakefile " + f.getAbsolutePath() + " is invalid and will be ignored: " +
						e.getMessage());
			}
		...
		// Build a dependency graph if wanted
		ModuleDependencyGraphOptions graphOptions = options.getDependencyGraphOptions();
		if(graphOptions != null) {
			if(graphOptions.getDotStream() == null)
				throw new IllegalArgumentException("Null dotStream specified in DependencyGraphOptions");
			String title = graphOptions.getTitle();
			if(title == null)
				title = "Module Dependencies";

			Class<? extends IHrefProducer> hrefClazz = getHrefProducerClass(graphOptions.getGraphHrefType());
			// unknown submonitor with 1000 cancelation checks per work(1)
			DependencyDataCalculator graphFactory = new DependencyDataCalculator(
				hrefClazz, graphOptions.getGraphHrefPrefix());
			ICancel cancel = new ProgressMonitorCancelIndicator(ticker.newChild(IProgressMonitor.UNKNOWN), 1000);

			graphFactory.produceGraph(cancel, root, //
				title, //
				graphOptions.getDotStream(), //
				graphOptions.getModulesToGraph(), //
				moduleData, //
				all);
		}
	...
	/**
	 * Validate module meta data if options has isCheckModuleSemantics (otherwise this validation
	 * is a no-op). A tick is produced on the monitor even if validation is not wanted.
